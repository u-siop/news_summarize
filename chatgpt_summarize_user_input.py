...
2. 각 문장의 종결어미 형식은 '-임', '-함', '-라고 함', '-다고 함', '-음', '-하는 중' 등 명사로 종결
3. 반드시 두 문장으로 해당 내용을 요약하고, 각 문장은 새로운 줄에서 시작해야 함
4. 제목 아이디어를 3~5개 추가적으로 만들고, 35자 이내로 작성
5. 위의 [형식]을 정확히 따라 작성
6. 대표해시태그는 뉴스 전체를 대표할 수 있는 3어절 이내의 대표해시태그를 추가
7. 대표해시태그 아이디어를 3개, 3어절 이내, 각 어절은 띄어쓰기로 구별
...
def main():
    ...
    file_name = f"news_summary_{current_time}.txt"
